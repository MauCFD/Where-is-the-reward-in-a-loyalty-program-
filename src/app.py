...
def index():
    ...
    all_data = mongo.db.reward_sales.find({},{'_id':False})
    response = json_util.dumps(all_data)

    return render_template('index.html',data =  Response(response, mimetype='application/json'))
...
def get_data():
    ...
    # my_data = []
    # for x in data:
    #     my_data.append(x['Client ID'])
    # return render_template('index.html', collection = my_data)
    # console.log(my_data)
